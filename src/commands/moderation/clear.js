  ...
  async execute(interaction, client) {
    ...
    if (toDelete < 1)
      return interaction.editReply(
        `Amount of messages to delete must be equal to 1 or greater.\nReceived: ${toDelete}`
      );
    let total = 0;
    let deleting = await timesRepeat(toDelete);
    const channel =
      (await interaction.options.getChannel("targetchannel")) ||
      (await client.channels.cache.get(interaction.channelId));
    let last;
    ...
    for (let i = deleting.length - 1; i > -1; i--) {
          ...
          if (user)
            final = [
              ...(await messages.filter(
                (m) => m.author.id == user.id && m.deletable
              )),
            ]
              .map((e) => e[1])
              .slice(0, numb);
      ...
      if (i == 0)
        await interaction.editReply({
          content: `Got rid of: ${total} messages ${plural(total)} ${
            user ? `that ${user} sent` : ``
          } from ${channel}`,
        });
    }
