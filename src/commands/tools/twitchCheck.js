...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName(`twitch_name`)
        .setDescription(
          "The twitch name can be found after twitch.tv/< twitch_name >"
        )
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName(`twitch_notification_role_id`)
        .setDescription(`Twitch notification role ID to ping`)
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
  ...
  async execute(interaction, client) {
    ...
    if (!guildProfile) {
      ...
      await guildProfile
        .updateOne({ twitchChannelName: myTwitchName })
        .catch(console.error);
      await guildProfile.save().catch(console.error);
      await guildProfile
        .updateOne({ guildTwitchChannel: myTwitchChannelId })
        .catch(console.error);
      await guildProfile.save().catch(console.error);
      await guildProfile
        .updateOne({ roleTwitchNotificationId: myTwitchNotificationRoleId })
        .catch(console.error);
      await guildProfile.save().catch(console.error);
    }
