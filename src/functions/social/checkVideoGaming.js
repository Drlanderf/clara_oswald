...
async function checkVideoGaming(client) {
    ...
    const data = await parser.parseURL(`https://youtube.com/feeds/videos.xml?channel_id=${MyYoutubeChannelID01}`).catch(console.error);
    if (guildProfile.lastVideo01 !== data.items[0].id) {
        console.log("[videoCheck_Gaming] NEW VIDEO spotted");
        await guildProfile.updateOne({ lastVideo01: data.items[0].id }).catch(console.error);
        await guildProfile.save().catch(console.error);

        const guild = await client.guilds.fetch(`${guildId}`).catch(console.error);
        const channel = await guild.channels.fetch(`${MyYoutubeGuildChannelID}`).catch(console.error);
