...
async function checkVideoTech(interaction, client) {
    ...
    const data = await parser.parseURL(`https://youtube.com/feeds/videos.xml?channel_id=${MyYoutubeChannelID00}`).catch(console.error);

    if (guildProfile.lastVideo00 !== data.items[0].id) {
        //new video or not sent
        await guildProfile.updateOne({ lastVideo00: data.items[0].id }).catch(console.error);
