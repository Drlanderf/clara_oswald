...
async function checkStreamTwitch(interaction, client) {
    ...
    const uptime = await fetch.get(`https://decapi.me/twitch/uptime/${myTwitchChannelName}`);
    const avatar = await fetch.get(`https://decapi.me/twitch/avatar/${myTwitchChannelName}`);
    const viewers = await fetch.get(`https://decapi.me/twitch/viewercount/${myTwitchChannelName}`);
    const title = await fetch.get(`https://decapi.me/twitch/title/${myTwitchChannelName}`);
    const game = await fetch.get(`https://decapi.me/twitch/game/${myTwitchChannelName}`);
    ...
    if (uptime.body !== `${myTwitchChannelName} is offline`) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setImage(`https://static-cdn.jtvnw.net/previews-ttv/live_user_${myTwitchChannelName}-620x378.jpg`)
            .setFooter({
